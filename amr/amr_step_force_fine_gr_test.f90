...
!    test_rho=0.0D0
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           ! Set field that we want to use
           igrp = 9
           gr_pot(icell_amr,7) = 0.0d0 
           gr_pot(icell_amr,8) = 0.0d0
           gr_pot(icell_amr,9) = 0.0d0
           gr_pot(icell_amr,10) = dsin(2.0d0*pii*xs)*dsin(2.0d0*pii*ys)*dsin(2.0d0*pii*zs)
...
!    if(verbose) write(*,*) 'The average density at a=',aexp,' is',test_rhobar
     ...
     ! Communicate gr_pot
     call make_virtual_fine_dp(gr_pot(1,7),ilevel)
     call make_virtual_fine_dp(gr_pot(1,8),ilevel)
     call make_virtual_fine_dp(gr_pot(1,9),ilevel)
     call make_virtual_fine_dp(gr_pot(1,10),ilevel)

     ! Call GR force calculation
     call force_fine_gr(ilevel,icount,igrp)

     ! Communicate force array
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           if((rr>0.1d0.and.rr<0.1d0+1.0d0/256.0d0).and.(zs>0.5D0.and.zs<0.5D0+1.0D0/256.0D0)) then
              write(*,'(6(f12.7,2x))') xs, ys, zs, f(icell_amr,1), f(icell_amr,2), f(icell_amr,3)
           end if    

           !if(zs>0.5D0.and.zs<0.5D0+1.0D0/256.0D0.and.ys>0.5D0.and.ys<0.5D0+1.0D0/256.0D0) then
           !   write(*,'(5(f12.6,2x))') xs, ys, f(icell_amr,1), f(icell_amr,2), f(icell_amr,3)
           !end if    
     ...
     if (myid==1) write(*,*) 'GR test finished for force_fine_gr. Now exiting.' 
     call clean_stop
  end if
