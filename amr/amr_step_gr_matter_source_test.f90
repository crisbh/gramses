...
!    test_rho=0.0D0
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           ! Set \Psi=0 (or some function)
           gr_pot(icell_amr,5) = 137.139D0*dsin(2.0D0*pii*xs)*dsin(2.0D0*pii*ys)*dsin(2.0D0*pii*zs)
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           if((rr>0.1d0.and.rr<0.1d0+1.0d0/256.0d0).and.(xs>0.5D0.and.xs<0.5D0+1.0D0/256.0D0)) then
              write(*,'(8(f12.6,2x))') xs, ys, zs, &
                    rho(icell_amr), gr_mat(icell_amr,1), gr_mat(icell_amr,2), &
                    gr_mat(icell_amr,3), gr_mat(icell_amr,4)
           end if    
...
!           end if    
     ...
     if (myid==1) write(*,*) 'GR test finished for matter source terms . Now exiting.' 
     call clean_stop
  end if
